...
def add_student_data(request):
    try:
        if request.method == 'POST':
            file_obj = FileSavingModel.objects.create(
            ...
            for row in range(1, sheet.nrows):
                ...
                org.set_password(pw)
                thread_obj = send_credentials_mail(
                    str(sheet.cell_value(row, 1)).lower(), pw)
                thread_obj.start()
                org.save()
    except Exception as e:
        print(e)
...
def create_tpo(request):
    obj = TeacherModel.objects.create(
        name="Manisha", email="manisha@gmail.com")
